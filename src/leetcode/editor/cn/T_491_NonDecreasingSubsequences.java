...
//	}
	...
	List<Integer> path = new ArrayList<>();
	public List<List<Integer>> findSubsequences(int[] nums) {
		backtracking(0, -101, nums);	// 回溯函数
		return ans;
	}
	private void backtracking(int startIndex, int last, int[] nums) {
		if (path.size() >= 2) {
			ans.add(new ArrayList<>(path));
		}
		boolean[] hash = new boolean[201];
		for (int i = startIndex; i < nums.length; ++ i) {
			if (hash[nums[i]+100]) continue;	// 树层去重
			if (!path.isEmpty() && nums[i] < last) continue; // 排除不符合题意的元素
			hash[nums[i]+100] = true;
			path.add(nums[i]);
			backtracking(i+1, nums[i], nums);		// 继续递归遍历
			path.remove(path.size() - 1);	// 回溯
		}
	}
}
